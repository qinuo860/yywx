      ...
      switch (p0.postType) {
        case PostType.reel:
          if (Get.isRegistered<ReelController>(tag: '${p0.id}')) {
            ...
            controller.updateReelData(reel: p0);
          }
          break;
        case PostType.image:
        case PostType.video:
        case PostType.text:
          if (Get.isRegistered<PostScreenController>(tag: '${p0.id}')) {
            ...
            controller.updatePost(p0); // Safe update after build
          }
          break;
        case PostType.none:
          break;
    ...
    if (commentHelper.isTextComment.value) {
      await commentHelper.onCommentPost(
          ...
          onUpdateComment: (comment, isReplyComment) {
            if (isReplyComment) {
              Loggers.success('isReply Comment');
              ...
              post.update((val) => val?.updateCommentCount(1));
              if (Get.isRegistered<PostScreenController>(
                  tag: '${post.value?.id}')) {
                PostScreenController controller =
                    Get.find<PostScreenController>(tag: '${post.value?.id}');
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  if (post.value != null) {
                    controller
                        .updatePost(post.value!); // Safe update after build
                  }
                });
              }
              // onUpdatePost(post.value);
            }
          });
    } else {
